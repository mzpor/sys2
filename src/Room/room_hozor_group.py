...
# لیست کاربران گروه (به جای کاربر1 تا 10)
group_users = []
attendance_data = {}
...
# تابع اضافه کردن کاربر به لیست
def add_user_to_list(user_info, chat_id):
    ...
    for existing_user in group_users:
...
def get_attendance_list():
    ...
    if not group_users:
        ...
        text += "از دستور /عضو برای اضافه کردن اعضا استفاده کنید."
        return text
    ...
    # ثبت سریع
        ...
        if not group_users:
            edit_message(chat_id, message_id, 
                        ...
                        "از دستور /عضو برای اضافه کردن اعضا استفاده کنید.", 
...
👥 تعداد اعضای ثبت شده: {len(group_users)}
...
لطفاً یکی از گزینه‌های زیر را انتخاب کنید:"""
        ...
        success, message_text = add_user_to_list(user_info, chat_id)
        send_message(chat_id, message_text)
        
        # نمایش لیست کامل اعضا
        if group_users:
            members_text = f"👥 **لیست کامل اعضای ثبت شده**\n\n"
            for i, user in enumerate(group_users, 1):
                members_text += f"{i}. {user['name']}\n"
            members_text += f"\n📊 تعداد کل: {len(group_users)} نفر"
