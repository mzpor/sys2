...
# تنظیمات Client برای Appwrite
client = Client()
client.set_endpoint('https://cloud.appwrite.io/v1')
client.set_project('687bb7e401f6ed0f305')  # مثال، مقدار واقعی رو جایگزین کن # ID پروژه‌ات رو از داشبورد کپی کن
client.set_key('your_api_key')  # API Key که ساختي

# اتصال به Storage
storage = Storage(client)
bucket_id = '6886627300168a7b942a'  # Bucket ID که دادی
...
def save_to_text_file(chat_id, user_info):
    """ذخیره اطلاعات کاربر توی فایل متنی و آپلود به Storage"""
    # ساخت محتوا برای فایل
    content = f"چت ID: {chat_id}\n"
    content += f"نام: {user_info.get('name', 'ندارد')}\n"
    content += f"شماره تلفن: {user_info.get('phone', 'ندارد')}\n"
    content += f"شماره ملی: {user_info.get('national_id', 'ندارد')}\n"
    content += f"کلاس انتخابی: {user_info.get('selected_class', 'ندارد')}\n"
    content += "-" * 30 + "\n"  # خط جداکننده
    ...
    # آپلود فایل به Appwrite Storage
    with open("user_data.txt", "rb") as file:
        storage.create_file(bucket_id, file, name=f"user_data_{chat_id}.txt")
    print(f"داده‌های کاربر {chat_id} توی Storage آپلود شد!")
...
def handle_payment_completion(chat_id):
    ...
    # ذخیره داده‌ها توی فایل متنی
    save_to_text_file(chat_id, user_info)
