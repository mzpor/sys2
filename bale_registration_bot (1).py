...
def load_data():
    """بارگذاری داده‌ها از فایل 1.json"""
    global users_data
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, 'r', encoding='utf-8') as f:
            users_data = json.load(f)
        logging.info("Data loaded from 1.json")
    else:
        users_data = {}
...
def save_data():
    """ذخیره داده‌ها در فایل 1.json"""
    with open(DATA_FILE, 'w', encoding='utf-8') as f:
        json.dump(users_data, f, ensure_ascii=False, indent=4)
    logging.info("Data saved to 1.json")
...
def validate_phone_number(phone_number):
    ...
    if re.match(r'^\+98\d{10}$', phone_number) and len(phone_number) == 13:
        return True, phone_number
    return False, "شماره تلفن نامعتبر است. باید با +98 شروع شود و ۱۳ کاراکتر باشد."
...
def process_message(message):
    ...
    if text == "برگشت به قبل" and users_data[user_id]["step"] != "start":
        ...
        elif users_data[user_id]["step"] == "final_confirmation":
            users_data[user_id]["step"] = "waiting_phone"
            save_data()
            send_message(chat_id, f"{users_data[user_id]['first_name']} عزیز، لطفاً برای ارسال شماره تلفن خود روی دکمه زیر بزنید.", 
                         reply_markup={"inline_keyboard": [[{"text": "📱 ارسال تلفن", "callback_data": "send_phone"}]]})
        logging.info(f"User {user_id} went back to previous step")
        return

    # معرفی آموزشگاه
    if text == "معرفی آموزشگاه":
        send_message(chat_id, "📚 آموزشگاه ما ارائه‌دهنده دوره‌های آموزشی با کیفیت است. برای ثبت‌نام، روی 'شروع ثبت‌نام' بزنید!", reply_markup=main_keyboard())
        logging.info(f"User {user_id} viewed introduction")
